...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    private static final class StreamReader implements Reader {
        ...
        /* renamed from: is */
        private final InputStream f113is;

        StreamReader(InputStream inputStream) {
            this.f113is = inputStream;
        }

        public int getUInt16() throws IOException {
            return ((this.f113is.read() << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (this.f113is.read() & 255);
        }

        public short getUInt8() throws IOException {
            return (short) (this.f113is.read() & 255);
        }
        ...
        public long skip(long j) throws IOException {
            ...
            long j2 = j;
            while (j2 > 0) {
                long skip = this.f113is.skip(j2);
                if (skip <= 0) {
                    if (this.f113is.read() == -1) {
                        break;
                    }
                    skip = 1;
                }
        ...
        public int read(byte[] bArr, int i) throws IOException {
            int i2 = i;
            while (i2 > 0) {
                int read = this.f113is.read(bArr, i - i2, i2);
                if (read == -1) {
                    break;
                }
                i2 -= read;
