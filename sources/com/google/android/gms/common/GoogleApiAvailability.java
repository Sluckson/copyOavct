        ...
        if (i == 18) {
            ...
            if (DeviceProperties.isWearable(context)) {
                ...
                if (DeviceProperties.isWearableWithoutPlayStore(context)) {
                    style.addAction(C1283R.C1285drawable.common_full_open_on_phone, resources.getString(C1283R.string.common_open_on_phone), pendingIntent);
                } else {
                    style.setContentIntent(pendingIntent);
                }
            } else {
                style.setSmallIcon(17301642).setTicker(resources.getString(C1283R.string.common_google_play_services_notification_ticker)).setWhen(System.currentTimeMillis()).setContentIntent(pendingIntent).setContentText(errorNotificationMessage);
            }
            if (PlatformVersion.isAtLeastO()) {
