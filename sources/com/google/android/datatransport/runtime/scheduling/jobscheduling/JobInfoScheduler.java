...
public class JobInfoScheduler implements WorkScheduler {
    ...
    public void schedule(TransportContext transportContext, int i) {
        ...
        if (isJobServiceOn(jobScheduler, jobId, i)) {
            Logging.m31d(LOG_TAG, "Upload for context %s is already scheduled. Returning...", (Object) transportContext);
            return;
        }
        ...
        Logging.m33d(LOG_TAG, "Scheduling upload for context %s with jobId=%d in %dms(Backend next call timestamp %d). Attempt %d", transportContext, Integer.valueOf(jobId), Long.valueOf(this.config.getScheduleDelay(transportContext.getPriority(), nextCallTime, i)), Long.valueOf(nextCallTime), Integer.valueOf(i));
