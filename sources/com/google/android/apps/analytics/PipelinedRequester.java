...
class PipelinedRequester {
    ...
    private void maybeOpenConnection() throws IOException {
        ...
        if (defaultHttpClientConnection == null || !defaultHttpClientConnection.isOpen()) {
            ...
            Socket connectSocket = this.socketFactory.connectSocket(this.socketFactory.createSocket(), this.host.getHostName(), this.host.getPort(), (InetAddress) null, 0, basicHttpParams);
        ...
        while (metrics.getResponseCount() < metrics.getRequestCount()) {
            ...
            if (headers != null) {
                for (Header value : headers) {
                    if (PreviewActivity.ON_CLICK_LISTENER_CLOSE.equalsIgnoreCase(value.getValue())) {
                        this.callbacks.pipelineModeChanged(false);
                        this.canPipeline = false;
                    }
                }
