...
public final class PesReader implements TsPayloadReader {
    ...
    public final void consume(ParsableByteArray parsableByteArray, int i) throws ParserException {
        if ((i & 1) != 0) {
            ...
            if (!(i2 == 0 || i2 == 1)) {
                if (i2 == 2) {
                    Log.m54w(TAG, "Unexpected start indicator reading extended header");
                } else if (i2 == 3) {
                    if (this.payloadSize != -1) {
                        Log.m54w(TAG, "Unexpected start indicator: expected " + this.payloadSize + " more bytes");
                    }
    ...
    private boolean parseHeader() {
        ...
        if (readBits != 1) {
            Log.m54w(TAG, "Unexpected start code prefix: " + readBits);
            this.payloadSize = -1;
            return false;
        }
