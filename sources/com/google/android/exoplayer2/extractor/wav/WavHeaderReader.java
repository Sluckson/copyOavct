...
final class WavHeaderReader {
    ...
    public static WavHeader peek(ExtractorInput extractorInput) throws IOException, InterruptedException {
        ...
        if (readInt != WavUtil.WAVE_FOURCC) {
            Log.m50e(TAG, "Unsupported RIFF format: " + readInt);
            return null;
        }
        ...
        if (readLittleEndianUnsignedShort3 == i) {
            ...
            if (encodingForType == 0) {
                Log.m50e(TAG, "Unsupported WAV format: " + readLittleEndianUnsignedShort4 + " bit/sample, type " + readLittleEndianUnsignedShort);
                return null;
            }
    ...
    public static void skipToData(ExtractorInput extractorInput, WavHeader wavHeader) throws IOException, InterruptedException {
        ...
        ChunkHeader peek = ChunkHeader.peek(extractorInput, parsableByteArray);
        while (peek.f126id != WavUtil.DATA_FOURCC) {
            if (!(peek.f126id == WavUtil.RIFF_FOURCC || peek.f126id == WavUtil.FMT_FOURCC)) {
                Log.m54w(TAG, "Ignoring unknown WAV chunk: " + peek.f126id);
            }
            long j = peek.size + 8;
            if (peek.f126id == WavUtil.RIFF_FOURCC) {
                j = 12;
            }
            if (j <= 2147483647L) {
        ...
        if (length != -1 && j2 > length) {
            Log.m54w(TAG, "Data exceeds input length: " + j2 + ", " + length);
            j2 = length;
        }
