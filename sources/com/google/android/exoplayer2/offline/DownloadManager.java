    ...
    private static final class InternalHandler extends Handler {
        ...
        private void initialize(int i) {
            ...
            try {
                ...
                Log.m51e(DownloadManager.TAG, "Failed to load index.", e);
        ...
        private void setStopReason(@Nullable String str, int i) {
            if (str == null) {
                ...
                try {
                    ...
                    Log.m51e(DownloadManager.TAG, "Failed to set manual stop reason", e);
                }
            } else {
                ...
                if (download != null) {
                    ...
                    try {
                        ...
                        Log.m51e(DownloadManager.TAG, "Failed to set manual stop reason: " + str, e2);
                    }
                }
            }
            syncTasks();
        ...
        private void addDownload(DownloadRequest downloadRequest, int i) {
            Download download = getDownload(downloadRequest.f137id, true);
        ...
        private void removeDownload(String str) {
            ...
            if (download == null) {
                Log.m50e(DownloadManager.TAG, "Failed to remove nonexistent download: " + str);
                return;
            }
        ...
        private void removeAllDownloads() {
            ...
            try {
                ...
                Log.m50e(DownloadManager.TAG, "Failed to load downloads.");
            } catch (Throwable th) {
                r5.addSuppressed(th);
            }
            for (int i = 0; i < this.downloads.size(); i++) {
            ...
            Collections.sort(this.downloads, C1187x7c64c816.INSTANCE);
            try {
                this.downloadIndex.setStatesToRemoving();
            } catch (IOException e) {
                Log.m51e(DownloadManager.TAG, "Failed to update index.", e);
            }
        ...
        private void syncTasks() {
            ...
            for (int i2 = 0; i2 < this.downloads.size(); i2++) {
                ...
                Task task = this.activeTasks.get(download.request.f137id);
        ...
        private Task syncQueuedDownload(@Nullable Task task, Download download) {
            if (task != null) {
                ...
                this.activeTasks.put(putDownloadWithState.request.f137id, task2);
        ...
        private void syncRemovingDownload(@Nullable Task task, Download download) {
            if (task == null) {
                ...
                this.activeTasks.put(download.request.f137id, task2);
                task2.start();
        ...
        private void onContentLengthChanged(Task task) {
            String str = task.request.f137id;
        ...
        private void onTaskStopped(Task task) {
            ...
            if (access$500 != null) {
                Log.m51e(DownloadManager.TAG, "Task failed: " + task.request + ", " + access$000, access$500);
            }
            Download download = (Download) Assertions.checkNotNull(getDownload(str, false));
        ...
        private void onDownloadTaskStopped(Download download, @Nullable Throwable th) {
            ...
            this.downloads.remove(getDownloadIndex(download3.request.f137id));
            try {
        ...
        private void onRemoveTaskStopped(Download download) {
            ...
            this.downloads.remove(getDownloadIndex(download.request.f137id));
            try {
                this.downloadIndex.removeDownload(download.request.f137id);
            } catch (IOException unused) {
                Log.m50e(DownloadManager.TAG, "Failed to remove from database");
            }
        ...
        private Download putDownload(Download download) {
            ...
            int downloadIndex2 = getDownloadIndex(download.request.f137id);
            if (downloadIndex2 == -1) {
        ...
        private Download getDownload(String str, boolean z) {
            ...
            try {
                ...
                Log.m51e(DownloadManager.TAG, "Failed to load download: " + str, e);
                return null;
            }
        }
        ...
        private int getDownloadIndex(String str) {
            for (int i = 0; i < this.downloads.size(); i++) {
                if (this.downloads.get(i).request.f137id.equals(str)) {
                    return i;
                }
            }
            return -1;
