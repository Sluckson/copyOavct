...
public class DefaultDashChunkSource implements DashChunkSource {
    ...
    private long liveEdgeTimeUs = C1119C.TIME_UNSET;
    ...
    public DefaultDashChunkSource(LoaderErrorThrower loaderErrorThrower, DashManifest dashManifest, int i, int[] iArr, TrackSelection trackSelection2, int i2, DataSource dataSource2, long j, int i3, boolean z, List<Format> list, @Nullable PlayerEmsgHandler.PlayerTrackEmsgHandler playerTrackEmsgHandler2) {
    ...
    public void getNextChunk(long j, long j2, List<? extends MediaChunk> list, ChunkHolder chunkHolder) {
        ...
        if (this.fatalError == null) {
            ...
            long msToUs = C1119C.msToUs(this.manifest.availabilityStartTimeMs) + C1119C.msToUs(this.manifest.getPeriod(this.periodIndex).startMs) + j2;
            ...
            if (playerTrackEmsgHandler2 == null || !playerTrackEmsgHandler2.maybeRefreshManifestBeforeLoadingNextChunk(msToUs)) {
                ...
                long j6 = C1119C.TIME_UNSET;
                int i3 = (access$000 > C1119C.TIME_UNSET ? 1 : (access$000 == C1119C.TIME_UNSET ? 0 : -1));
                boolean z = i3 != 0;
                ...
                int i4 = (segmentNum2 > j8 ? 1 : (segmentNum2 == j8 ? 0 : -1));
                if (i4 > 0 || (this.missingLastSegment && i4 >= 0)) {
                    ...
                    chunkHolder2.chunk = newMediaChunk(representationHolder3, this.dataSource, this.trackType, this.trackSelection.getSelectedFormat(), this.trackSelection.getSelectionReason(), this.trackSelection.getSelectionData(), segmentNum2, i5, j6);
                } else {
    ...
    private void updateLiveEdgeTimeUs(RepresentationHolder representationHolder, long j) {
        this.liveEdgeTimeUs = this.manifest.dynamic ? representationHolder.getSegmentEndTimeUs(j) : C1119C.TIME_UNSET;
    }
    ...
    private long resolveTimeToLiveEdgeUs(long j) {
        if (this.manifest.dynamic && this.liveEdgeTimeUs != C1119C.TIME_UNSET) {
    ...
    public Chunk newMediaChunk(RepresentationHolder representationHolder, DataSource dataSource2, int i, Format format, int i2, Object obj, long j, int i3, long j2) {
        ...
        long j4 = (access$000 == C1119C.TIME_UNSET || access$000 > segmentEndTimeUs) ? -9223372036854775807L : access$000;
    ...
    protected static final class RepresentationHolder {
        ...
        public long getFirstAvailableSegmentNum(DashManifest dashManifest, int i, long j) {
            if (getSegmentCount() != -1 || dashManifest.timeShiftBufferDepthMs == C1119C.TIME_UNSET) {
                return getFirstSegmentNum();
            }
            return Math.max(getFirstSegmentNum(), getSegmentNum(((j - C1119C.msToUs(dashManifest.availabilityStartTimeMs)) - C1119C.msToUs(dashManifest.getPeriod(i).startMs)) - C1119C.msToUs(dashManifest.timeShiftBufferDepthMs)));
        }
        ...
        public long getLastAvailableSegmentNum(DashManifest dashManifest, int i, long j) {
            ...
            if (segmentCount == -1) {
                firstSegmentNum = getSegmentNum((j - C1119C.msToUs(dashManifest.availabilityStartTimeMs)) - C1119C.msToUs(dashManifest.getPeriod(i).startMs));
            } else {
