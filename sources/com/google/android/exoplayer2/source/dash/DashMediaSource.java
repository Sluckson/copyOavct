...
public final class DashMediaSource extends BaseMediaSource {
    ...
    private DashMediaSource(DashManifest dashManifest, Uri uri, DataSource.Factory factory, ParsingLoadable.Parser<? extends DashManifest> parser, DashChunkSource.Factory factory2, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory2, LoadErrorHandlingPolicy loadErrorHandlingPolicy2, long j, boolean z, @Nullable Object obj) {
        ...
        this.expiredManifestPublishTimeUs = C1119C.TIME_UNSET;
    ...
    public MediaPeriod createPeriod(MediaSource.MediaPeriodId mediaPeriodId, Allocator allocator, long j) {
        ...
        this.periodsById.put(dashMediaPeriod.f142id, dashMediaPeriod);
    ...
    public void releasePeriod(MediaPeriod mediaPeriod) {
        ...
        this.periodsById.remove(dashMediaPeriod.f142id);
    }
    ...
    public void onManifestLoadCompleted(com.google.android.exoplayer2.upstream.ParsingLoadable<com.google.android.exoplayer2.source.dash.manifest.DashManifest> r16, long r17, long r19) {
        ...
        L_0x0047:
            ...
            com.google.android.exoplayer2.util.Log.m54w(r5, r8)
        L_0x005b:
            r5 = 1
            goto L_0x0098
        L_0x005d:
            ...
            com.google.android.exoplayer2.util.Log.m54w(r8, r5)
            goto L_0x005b
        L_0x0097:
            r5 = 0
        L_0x0098:
    ...
    private void onUtcTimestampResolutionError(IOException iOException) {
        Log.m51e(TAG, "Failed to resolve UtcTiming element.", iOException);
        processManifest(true);
    }
    ...
    private void processManifest(boolean z) {
        ...
        if (!this.manifest.dynamic || createPeriodSeekInfo2.isIndexExplicit) {
            ...
            j5 = Math.min((getNowUnixTimeUs() - C1119C.msToUs(this.manifest.availabilityStartTimeMs)) - C1119C.msToUs(this.manifest.getPeriod(periodCount).startMs), j5);
            if (this.manifest.timeShiftBufferDepthMs != C1119C.TIME_UNSET) {
                long msToUs = j5 - C1119C.msToUs(this.manifest.timeShiftBufferDepthMs);
        ...
        if (this.manifest.dynamic) {
            ...
            if (!this.livePresentationDelayOverridesManifest && this.manifest.suggestedPresentationDelayMs != C1119C.TIME_UNSET) {
                j7 = this.manifest.suggestedPresentationDelayMs;
            }
            long msToUs2 = j6 - C1119C.msToUs(j7);
        ...
        refreshSourceInfo(new DashTimeline(this.manifest.availabilityStartTimeMs, this.manifest.availabilityStartTimeMs + this.manifest.getPeriod(0).startMs + C1119C.usToMs(j), this.firstPeriodId, j, j6, j2, this.manifest, this.tag), this.manifest);
        if (!this.sideloadedManifest) {
            this.handler.removeCallbacks(this.simulateManifestRefreshRunnable);
            if (z2) {
            ...
            if (this.manifestLoadPending) {
                startLoadingManifest();
            } else if (z && this.manifest.dynamic && this.manifest.minUpdatePeriodMs != C1119C.TIME_UNSET) {
    ...
    private long getNowUnixTimeUs() {
        if (this.elapsedRealtimeOffsetMs != 0) {
            return C1119C.msToUs(SystemClock.elapsedRealtime() + this.elapsedRealtimeOffsetMs);
        }
        return C1119C.msToUs(System.currentTimeMillis());
    }
    ...
    private static final class DashTimeline extends Timeline {
        ...
        public Timeline.Period getPeriod(int i, Timeline.Period period, boolean z) {
            ...
            Integer num = null;
            String str = z ? this.manifest.getPeriod(i).f145id : null;
            if (z) {
                num = Integer.valueOf(this.firstPeriodId + i);
            }
            return period.set(str, num, 0, this.manifest.getPeriodDurationUs(i), C1119C.msToUs(this.manifest.getPeriod(i).startMs - this.manifest.getPeriod(0).startMs) - this.offsetInFirstPeriodUs);
        }

        public Timeline.Window getWindow(int i, Timeline.Window window, boolean z, long j) {
            Assertions.checkIndex(i, 0, 1);
            return window.set(z ? this.windowTag : null, this.presentationStartTimeMs, this.windowStartTimeMs, true, this.manifest.dynamic && this.manifest.minUpdatePeriodMs != C1119C.TIME_UNSET && this.manifest.durationMs == C1119C.TIME_UNSET, getAdjustedWindowDefaultStartPositionUs(j), this.windowDurationUs, 0, getPeriodCount() - 1, this.offsetInFirstPeriodUs);
        }
