...
public class DashManifest implements FilterableManifest<DashManifest> {
    ...
    public final long getPeriodDurationUs(int i) {
        return C1119C.msToUs(getPeriodDurationMs(i));
    }
    ...
    public final DashManifest copy(List<StreamKey> list) {
        ...
        while (true) {
            ...
            if (((StreamKey) linkedList.peek()).periodIndex != i) {
                ...
                if (periodDurationMs != C1119C.TIME_UNSET) {
                    j2 += periodDurationMs;
                }
            } else {
                Period period = getPeriod(i);
                arrayList.add(new Period(period.f145id, period.startMs - j2, copyAdaptationSets(period.adaptationSets, linkedList), period.eventStreams));
            }
            i++;
        }
        ...
        do {
            ...
            do {
                ...
                if (!(poll.periodIndex == i && poll.groupIndex == i2)) {
                    arrayList.add(new AdaptationSet(adaptationSet.f143id, adaptationSet.type, arrayList2, adaptationSet.accessibilityDescriptors, adaptationSet.supplementalProperties));
                }
                arrayList2.add(list2.get(poll.trackIndex));
                poll = linkedList.poll();
                break;
