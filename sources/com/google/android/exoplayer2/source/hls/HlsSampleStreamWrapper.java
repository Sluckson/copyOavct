...
final class HlsSampleStreamWrapper implements Loader.Callback<Chunk>, Loader.ReleaseCallback, SequenceableLoader, ExtractorOutput, SampleQueue.UpstreamFormatChangedListener {
    ...
    public int readData(int i, FormatHolder formatHolder, DecoderInputBuffer decoderInputBuffer, boolean z) {
        ...
        if (read == -5) {
            ...
            if (i == this.primarySampleQueueIndex) {
                ...
                format2 = format2.copyWithManifestFormatInfo(i2 < this.mediaChunks.size() ? this.mediaChunks.get(i2).trackFormat : this.upstreamTrackFormat);
            }
            if (!(format2.drmInitData == null || (drmInitData = this.overridingDrmInitData.get(format2.drmInitData.schemeType)) == null)) {
    ...
    public Loader.LoadErrorAction onLoadError(Chunk chunk, long j, long j2, IOException iOException, int i) {
        ...
        boolean z = false;
        boolean maybeBlacklistTrack = blacklistDurationMsFor != C1119C.TIME_UNSET ? this.chunkSource.maybeBlacklistTrack(chunk2, blacklistDurationMsFor) : false;
        if (maybeBlacklistTrack) {
            ...
            createRetryAction = retryDelayMsFor != C1119C.TIME_UNSET ? Loader.createRetryAction(false, retryDelayMsFor) : Loader.DONT_RETRY_FATAL;
        }
    ...
    private static Format deriveFormat(Format format, Format format2, boolean z) {
        ...
        return format2.copyWithContainerInfo(format.f120id, format.label, mediaMimeType, codecsOfType, format.metadata, i, format.width, format.height, i2, format.selectionFlags, format.language);
    }
    ...
    private static DummyTrackOutput createDummyTrackOutput(int i, int i2) {
        Log.m54w(TAG, "Unmapped track with id " + i + " of type " + i2);
