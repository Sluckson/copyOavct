...
public final class ClippingMediaSource extends CompositeMediaSource<Void> {
    ...
    public long getMediaTimeForChildMediaTime(Void voidR, long j) {
        ...
        long usToMs = C1119C.usToMs(this.startUs);
        long max = Math.max(0, j - usToMs);
        long j2 = this.endUs;
        return j2 != Long.MIN_VALUE ? Math.min(C1119C.usToMs(j2) - usToMs, max) : max;
    }
    ...
    private static final class ClippingTimeline extends ForwardingTimeline {
        ...
        public ClippingTimeline(Timeline timeline, long j, long j2) throws IllegalClippingException {
            ...
            if (timeline.getPeriodCount() == 1) {
                ...
                if (window.durationUs != C1119C.TIME_UNSET) {
                ...
                this.startUs = max;
                this.endUs = max2;
                int i = (max2 > C1119C.TIME_UNSET ? 1 : (max2 == C1119C.TIME_UNSET ? 0 : -1));
                this.durationUs = i == 0 ? -9223372036854775807L : max2 - max;
                if (window.isDynamic && (i == 0 || (window.durationUs != C1119C.TIME_UNSET && max2 == window.durationUs))) {
                    z = true;
                }
                this.isDynamic = z;
                return;
        ...
        public Timeline.Window getWindow(int i, Timeline.Window window, boolean z, long j) {
            ...
            if (window.defaultPositionUs != C1119C.TIME_UNSET) {
                window.defaultPositionUs = Math.max(window.defaultPositionUs, this.startUs);
                if (this.endUs == C1119C.TIME_UNSET) {
            ...
            long usToMs = C1119C.usToMs(this.startUs);
            if (window.presentationStartTimeMs != C1119C.TIME_UNSET) {
                window.presentationStartTimeMs += usToMs;
            }
            if (window.windowStartTimeMs != C1119C.TIME_UNSET) {
        ...
        public Timeline.Period getPeriod(int i, Timeline.Period period, boolean z) {
            ...
            return period.set(period.f122id, period.uid, 0, j == C1119C.TIME_UNSET ? -9223372036854775807L : j - positionInWindowUs, positionInWindowUs);
        }
    }
}
