...
public final class Cea708Decoder extends CeaDecoder {
            ...
            if ((i == 2 || i == 3) && z2) {
                if (i == 3) {
                    ...
                    if (dtvCcPacket2 == null) {
                        Log.m50e(TAG, "Encountered DTVCC_PACKET_DATA before DTVCC_PACKET_START");
                    } else {
    ...
    private void processCurrentPacket() {
        if (this.currentDtvCcPacket.currentIndex != (this.currentDtvCcPacket.packetSize * 2) - 1) {
            Log.m54w(TAG, "DtvCcPacket ended prematurely; size is " + ((this.currentDtvCcPacket.packetSize * 2) - 1) + ", but current index is " + this.currentDtvCcPacket.currentIndex + " (sequence number " + this.currentDtvCcPacket.sequenceNumber + "); ignoring packet");
            return;
        }
        ...
        if (readBits == 7) {
            ...
            if (readBits < 7) {
                Log.m54w(TAG, "Invalid extended service number: " + readBits);
            }
        }
        if (readBits2 == 0) {
            if (readBits != 0) {
                Log.m54w(TAG, "serviceNumber is non-zero (" + readBits + ") when blockSize is 0");
            }
            ...
            while (this.serviceBlockPacket.bitsLeft() > 0) {
                ...
                if (readBits3 == 16) {
                    ...
                    if (readBits4 <= 31) {
                        ...
                        Log.m54w(TAG, "Invalid extended command: " + readBits4);
                    }
                    ...
                    Log.m54w(TAG, "Invalid base command: " + readBits3);
                }
                z = true;
            }
            if (z) {
    ...
    private void handleC0Command(int i) {
        ...
        if (i == 3) {
            ...
            switch (i) {
                ...
                default:
                    if (i >= 17 && i <= 23) {
                        Log.m54w(TAG, "Currently unsupported COMMAND_EXT1 Command: " + i);
                        this.serviceBlockPacket.skipBits(8);
                        return;
                    } else if (i < 24 || i > 31) {
                        Log.m54w(TAG, "Invalid C0 command: " + i);
                        return;
                    } else {
                        Log.m54w(TAG, "Currently unsupported COMMAND_P16 Command: " + i);
    ...
    private void handleC1Command(int r5) {
        ...
        L_0x0008:
            ...
            com.google.android.exoplayer2.util.Log.m54w(r0, r5)
            goto L_0x0117
        L_0x0020:
            int r5 = r5 + -152
    ...
    private void handleG2Character(int i) {
        if (i == 32) {
            ...
            this.currentCueBuilder.append(Typography.f5414tm);
        } else if (i == 58) {
                ...
                default:
                    switch (i) {
                        ...
                        case 127:
                            ...
                            return;
                        default:
                            Log.m54w(TAG, "Invalid G2 character: " + i);
                            return;
                    }
            }
        } else {
    ...
    private void handleG3Character(int i) {
        ...
        Log.m54w(TAG, "Invalid G3 character: " + i);
