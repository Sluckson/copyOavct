...
public abstract class LinearTransformation {
    ...
    public static final class LinearTransformationBuilder {
        ...
        /* renamed from: x1 */
        private final double f343x1;

        /* renamed from: y1 */
        private final double f344y1;
        ...
        public LinearTransformation and(double d, double d2) {
            boolean z = true;
            Preconditions.checkArgument(DoubleUtils.isFinite(d) && DoubleUtils.isFinite(d2));
            double d3 = this.f343x1;
            if (d != d3) {
                return withSlope((d2 - this.f344y1) / (d - d3));
            }
            if (d2 == this.f344y1) {
                z = false;
            }
            Preconditions.checkArgument(z);
            return new VerticalLinearTransformation(this.f343x1);
        }

        public LinearTransformation withSlope(double d) {
            Preconditions.checkArgument(!Double.isNaN(d));
            if (DoubleUtils.isFinite(d)) {
                return new RegularLinearTransformation(d, this.f344y1 - (this.f343x1 * d));
            }
            return new VerticalLinearTransformation(this.f343x1);
        }
    }
    ...
    private static final class VerticalLinearTransformation extends LinearTransformation {
        ...
        public String toString() {
            return String.format("x = %g", new Object[]{Double.valueOf(this.f345x)});
        }

        private LinearTransformation createInverse() {
            return new RegularLinearTransformation(FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE, this.f345x, this);
        }
    }
