...
public class DefaultSettingsSpiCall extends AbstractSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        if (z) {
            try {
                ...
                logger2.mo33257d("Requesting settings from " + getUrl());
                Logger logger3 = this.logger;
                logger3.mo33257d("Settings query params were: " + queryParamsFor);
                HttpResponse execute = applyHeadersTo.execute();
                Logger logger4 = this.logger;
                logger4.mo33257d("Settings request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
                return handleResponse(execute);
            } catch (IOException e) {
                this.logger.mo33260e("Settings request failed.", e);
                return null;
            }
        } else {
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger2 = this.logger;
        logger2.mo33257d("Settings result was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger3 = this.logger;
        logger3.mo33259e("Failed to retrieve settings from " + getUrl());
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            Logger logger2 = this.logger;
            logger2.mo33258d("Failed to parse settings JSON from " + getUrl(), e);
            Logger logger3 = this.logger;
            logger3.mo33257d("Settings response " + str);
            return null;
        }
    }
