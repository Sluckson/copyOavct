...
public class CrashlyticsCore {
    ...
    public boolean onPreExecute(SettingsDataProvider settingsDataProvider) {
        ...
        logger.mo33257d("Mapping file ID is: " + mappingFileId);
        if (isBuildIdValid(mappingFileId, CommonUtils.getBooleanResourceValue(this.context, CRASHLYTICS_REQUIRE_BUILD_ID, true))) {
            ...
            try {
                ...
                logger2.mo33261i("Initializing Crashlytics " + getVersion());
                ...
                logger3.mo33257d("Installer package name is: " + create.installerPackageName);
                ...
                if (!didPreviousInitializationFail || !CommonUtils.canTryConnection(this.context)) {
                    Logger.getLogger().mo33257d("Exception handling initialization successful");
                    return true;
                }
                Logger.getLogger().mo33257d("Crashlytics did not finish previous background initialization. Initializing synchronously.");
                finishInitSynchronously(settingsDataProvider);
                return false;
            } catch (Exception e) {
                Logger.getLogger().mo33260e("Crashlytics was not started due to an exception during initialization", e);
    ...
    public Task<Void> doBackgroundInitialization(SettingsDataProvider settingsDataProvider) {
        ...
        try {
            ...
            if (!settings.getFeaturesData().collectReports) {
                Logger.getLogger().mo33257d("Collection of crash reports disabled in Crashlytics settings.");
                return Tasks.forException(new RuntimeException("Collection of crash reports disabled in Crashlytics settings."));
            }
            if (!this.controller.finalizeSessions(settings.getSessionData().maxCustomExceptionEvents)) {
                Logger.getLogger().mo33257d("Could not finalize previous sessions.");
            }
            ...
            Logger.getLogger().mo33260e("Crashlytics encountered a problem during asynchronous initialization.", e);
    ...
    private void finishInitSynchronously(final SettingsDataProvider settingsDataProvider) {
        ...
        Logger.getLogger().mo33257d("Crashlytics detected incomplete initialization on previous app launch. Will initialize synchronously.");
        try {
            submit.get(4, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Logger.getLogger().mo33260e("Crashlytics was interrupted during initialization.", e);
        } catch (ExecutionException e2) {
            Logger.getLogger().mo33260e("Problem encountered during Crashlytics initialization.", e2);
        } catch (TimeoutException e3) {
            Logger.getLogger().mo33260e("Crashlytics timed out during initialization.", e3);
        }
    }
    ...
    public void markInitializationStarted() {
        ...
        Logger.getLogger().mo33257d("Initialization marker file created.");
    }
        ...
        this.backgroundWorker.submit(new Callable<Boolean>() {
            public Boolean call() throws Exception {
                try {
                    ...
                    Logger logger = Logger.getLogger();
                    logger.mo33257d("Initialization marker file removed: " + remove);
                    return Boolean.valueOf(remove);
                } catch (Exception e) {
                    Logger.getLogger().mo33260e("Problem encountered deleting Crashlytics initialization marker.", e);
                    return false;
                }
            }
        });
    ...
    static boolean isBuildIdValid(String str, boolean z) {
        if (!z) {
            Logger.getLogger().mo33257d("Configured not to require a build ID.");
            return true;
