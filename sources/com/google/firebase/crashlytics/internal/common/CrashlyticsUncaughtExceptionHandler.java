...
class CrashlyticsUncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable th) {
        ...
        if (thread == null) {
            try {
                Logger.getLogger().mo33259e("Could not handle uncaught exception; null thread");
            } catch (Exception e) {
                Logger.getLogger().mo33260e("An error occurred in the uncaught exception handler", e);
            } catch (Throwable th2) {
                Logger.getLogger().mo33257d("Crashlytics completed exception processing. Invoking default exception handler.");
                this.defaultHandler.uncaughtException(thread, th);
                this.isHandlingException.set(false);
                throw th2;
            }
        } else if (th == null) {
            Logger.getLogger().mo33259e("Could not handle uncaught exception; null throwable");
        } else {
