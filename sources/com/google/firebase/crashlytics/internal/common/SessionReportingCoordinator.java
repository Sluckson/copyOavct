...
class SessionReportingCoordinator implements CrashlyticsLifecycleEvents {
    ...
    public void persistFatalEvent(@NonNull Throwable th, @NonNull Thread thread, @NonNull String str, long j) {
        ...
        logger.mo33257d("Persisting fatal event for session " + str);
    ...
    public void persistNonFatalEvent(@NonNull Throwable th, @NonNull Thread thread, @NonNull String str, long j) {
        Logger logger = Logger.getLogger();
        logger.mo33257d("Persisting non-fatal event for session " + str);
    ...
    public void persistUserId(@NonNull String str) {
        ...
        if (userId == null) {
            Logger.getLogger().mo33257d("Could not persist user ID; no user ID available");
        } else {
    ...
    public Task<Void> sendReports(@NonNull Executor executor, @NonNull DataTransportState dataTransportState) {
        if (dataTransportState == DataTransportState.NONE) {
            Logger.getLogger().mo33257d("Send via DataTransport disabled. Removing DataTransport reports.");
            this.reportPersistence.deleteAllReports();
        ...
        for (CrashlyticsReportWithSessionId next : loadFinalizedReports) {
            if (next.getReport().getType() != CrashlyticsReport.Type.NATIVE || dataTransportState == DataTransportState.ALL) {
                ...
                Logger.getLogger().mo33257d("Send native reports via DataTransport disabled. Removing DataTransport reports.");
    ...
    private void persistEvent(@NonNull Throwable th, @NonNull Thread thread, @NonNull String str, @NonNull String str2, long j, boolean z) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().mo33257d("No log data to include with this event.");
        }
    ...
    public boolean onReportSendComplete(@NonNull Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            Logger logger = Logger.getLogger();
            logger.mo33257d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
            this.reportPersistence.deleteFinalizedReport(result.getSessionId());
            return true;
        }
        Logger.getLogger().mo33258d("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }
