...
public class ReportManager {
    ...
    public List<Report> findReports() {
        Logger.getLogger().mo33257d("Checking for crash reports...");
        File[] completeSessionFiles = this.reportFilesProvider.getCompleteSessionFiles();
        File[] nativeReportFiles = this.reportFilesProvider.getNativeReportFiles();
        LinkedList linkedList = new LinkedList();
        if (completeSessionFiles != null) {
            for (File file : completeSessionFiles) {
                Logger.getLogger().mo33257d("Found crash report " + file.getPath());
        ...
        if (linkedList.isEmpty()) {
            Logger.getLogger().mo33257d("No reports found.");
        }
        return linkedList;
    }
