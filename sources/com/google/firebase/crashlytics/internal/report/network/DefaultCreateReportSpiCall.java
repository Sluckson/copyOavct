...
public class DefaultCreateReportSpiCall extends AbstractSpiCall implements CreateReportSpiCall {
    ...
    public boolean invoke(CreateReportRequest createReportRequest, boolean z) {
        if (z) {
            ...
            logger.mo33257d("Sending report to: " + getUrl());
            try {
                HttpResponse execute = applyMultipartDataTo.execute();
                int code = execute.code();
                Logger logger2 = Logger.getLogger();
                logger2.mo33257d("Create report request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
                Logger logger3 = Logger.getLogger();
                logger3.mo33257d("Result was: " + code);
                return ResponseParser.parse(code) == 0;
            } catch (IOException e) {
                Logger.getLogger().mo33260e("Create report HTTP request failed.", e);
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest httpRequest, Report report) {
        ...
        if (report.getFiles().length == 1) {
            Logger.getLogger().mo33257d("Adding single file " + report.getFileName() + " to report " + report.getIdentifier());
            return part.part(FILE_PARAM, report.getFileName(), "application/octet-stream", report.getFile());
        }
        int i = 0;
        for (File file : report.getFiles()) {
            Logger.getLogger().mo33257d("Adding file " + file.getName() + " to report " + report.getIdentifier());
