...
public class DisplayCallbacksImpl implements FirebaseInAppMessagingDisplayCallbacks {
    ...
    public Task<Void> impressionDetected() {
        ...
        return maybeToTask(logToImpressionStore().andThen((CompletableSource) Completable.fromAction(DisplayCallbacksImpl$$Lambda$1.lambdaFactory$(this))).andThen((CompletableSource) updateWasImpressed()).toMaybe(), this.schedulers.mo34832io());
    }
    ...
    public Task<Void> displayErrorEncountered(FirebaseInAppMessagingDisplayCallbacks.InAppMessagingErrorReason inAppMessagingErrorReason) {
        if (shouldLog()) {
            ...
            return maybeToTask(logToImpressionStore().andThen((CompletableSource) Completable.fromAction(DisplayCallbacksImpl$$Lambda$5.lambdaFactory$(this, inAppMessagingErrorReason))).andThen((CompletableSource) updateWasImpressed()).toMaybe(), this.schedulers.mo34832io());
        }
    ...
    private Task<Void> logImpressionIfNeeded(Completable completable) {
        ...
        return maybeToTask(completable.toMaybe(), this.schedulers.mo34832io());
    }
