...
final class MarkableInputStream extends InputStream {
    ...
    /* renamed from: in */
    private final InputStream f3453in;
    private long limit;
    ...
    private MarkableInputStream(InputStream inputStream, int i, int i2) {
        ...
        this.f3453in = !inputStream.markSupported() ? new BufferedInputStream(inputStream, i) : inputStream;
    ...
    private void setLimit(long j) {
        try {
            if (this.reset >= this.offset || this.offset > this.limit) {
                ...
                this.f3453in.mark((int) (j - this.offset));
            } else {
                this.f3453in.reset();
                this.f3453in.mark((int) (j - this.reset));
    ...
    private void skip(long j, long j2) throws IOException {
        while (j < j2) {
            long skip = this.f3453in.skip(j2 - j);
            if (skip == 0) {
                if (read() != -1) {
                    skip = 1;
                } else {
    ...
    public int read(byte[] bArr) throws IOException {
        ...
        int read = this.f3453in.read(bArr);
        if (read != -1) {
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        ...
        int read = this.f3453in.read(bArr, i, i2);
        if (read != -1) {
            this.offset += (long) read;
        }
        return read;
    ...
    public long skip(long j) throws IOException {
        ...
        long skip = this.f3453in.skip(j);
        this.offset += skip;
        return skip;
    }

    public int available() throws IOException {
        return this.f3453in.available();
    }
    ...
    public boolean markSupported() {
        return this.f3453in.markSupported();
    }
}
