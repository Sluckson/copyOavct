...
public class LostPreBidSortActivity extends AbstractSortNFilter {
    ...
    public void createFiltersNOptions() {
        ...
        FilterSelect createSelectListWithImage7 = createSelectListWithImage(getString(C2723R.string.lbl_srt_bid_amount), false, getString(C2723R.string.lbl_srt_bid_amount), C2723R.C2725drawable.icon_sort_num_asc);
        FilterSelect createSelectListWithImage8 = createSelectListWithImage(getString(C2723R.string.lbl_srt_bid_amount), false, getString(C2723R.string.lbl_srt_bid_amount), C2723R.C2725drawable.icon_sort_num_desc);
        FilterSelect createSelectListWithImage9 = createSelectListWithImage(getString(C2723R.string.lbl_srt_branch), false, getString(C2723R.string.lbl_srt_branch), C2723R.C2725drawable.icon_sort_alpha_asc);
        FilterSelect createSelectListWithImage10 = createSelectListWithImage(getString(C2723R.string.lbl_srt_branch), false, getString(C2723R.string.lbl_srt_branch), C2723R.C2725drawable.icon_sort_alpha_desc);
        ...
        this.listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long j) {
                if (LostPreBidSortActivity.this.mTwoPane) {
                    Bundle bundle = new Bundle();
                    if (i == 0) {
                        LostPreBidSortActivity.this.filterSort1.isSelected = true;
                        LostPreBidSortActivity.this.filterSort1.filterResID = C2723R.C2725drawable.img_sort;
                        bundle.putParcelableArrayList("arraylist", LostPreBidSortActivity.this.filterSort1.filterSelects);
                        bundle.putInt(Constants.LISTING_TYPE, LostPreBidSortActivity.this.listType);
                        LostPreBidSortActivity.this.sortFragment.getArguments().putAll(bundle);
                        LostPreBidSortActivity.this.getSupportFragmentManager().beginTransaction().replace(C2723R.C2726id.item_detail_container, LostPreBidSortActivity.this.sortFragment).commit();
                        LostPreBidSortActivity.this.adapter.notifyDataSetChanged();
                    }
                }
            }
        });
    ...
    public void resetFilters() {
        ...
        if (this.sortFragment.filterSelects != null) {
            for (int i = 0; i < this.sortFragment.filterSelects.size(); i++) {
                if (i == Constants.DEFAULT_SORT_OPTION_FOR_LOSTPREBID) {
        ...
        int i2 = Constants.DEFAULT_SORT_OPTION_FOR_LOSTPREBID;
    ...
    public void applyFilters() {
        ...
        while (true) {
            ...
            if (i < this.sortFragment.filterSelects.size()) {
                if (this.sortFragment.filterSelects.get(i).getSel_isSelected().equalsIgnoreCase("true") && this.listType == Constants.LIST_LOST_PREBID) {
                    switch (i) {
                        case 0:
                        case 1:
                            str = "BidAmount";
