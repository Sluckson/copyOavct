...
import com.braintreepayments.api.googlepayment.C0964R;
...
public class GooglePayment {
        ...
        try {
            ...
            braintreeFragment.waitForConfiguration(new ConfigurationListener() {
                public void onConfigurationFetched(Configuration configuration) {
                    ...
                    for (String put : configuration.getGooglePayment().getSupportedNetworks()) {
                        jSONArray.put(put);
                    }
                    try {
    ...
    static int getEnvironment(GooglePaymentConfiguration googlePaymentConfiguration) {
        return BuildConfig.FLAVOR.equals(googlePaymentConfiguration.getEnvironment()) ? 1 : 3;
    }
    ...
    private static JSONObject buildPayPalPaymentMethodParameters(BraintreeFragment braintreeFragment) {
        JSONObject jSONObject = new JSONObject();
        try {
            jSONObject.put("purchase_context", new JSONObject().put("purchase_units", new JSONArray().put(new JSONObject().put("payee", new JSONObject().put("client_id", braintreeFragment.getConfiguration().getPayPal().getClientId())).put("recurring_payment", "true"))));
        } catch (JSONException unused) {
        }
        return jSONObject;
    }
    ...
    private static JSONObject buildCardTokenizationSpecification(BraintreeFragment braintreeFragment) {
        ...
        try {
            jSONObject2.put("gateway", "braintree").put("braintree:apiVersion", "v1").put("braintree:sdkVersion", "3.0.0").put("braintree:merchantId", braintreeFragment.getConfiguration().getMerchantId()).put("braintree:metadata", new JSONObject().put(FirebaseAnalytics.Param.SOURCE, "client").put("integration", braintreeFragment.getIntegrationType()).put("sessionId", braintreeFragment.getSessionId()).put(ClientCookie.VERSION_ATTR, "3.0.0").put("platform", "android").toString());
    ...
    private static JSONObject buildPayPalTokenizationSpecification(BraintreeFragment braintreeFragment) {
        JSONObject jSONObject = new JSONObject();
        try {
            jSONObject.put("type", "PAYMENT_GATEWAY").put("parameters", new JSONObject().put("gateway", "braintree").put("braintree:apiVersion", "v1").put("braintree:sdkVersion", "3.0.0").put("braintree:merchantId", braintreeFragment.getConfiguration().getMerchantId()).put("braintree:paypalClientId", braintreeFragment.getConfiguration().getPayPal().getClientId()).put("braintree:metadata", new JSONObject().put(FirebaseAnalytics.Param.SOURCE, "client").put("integration", braintreeFragment.getIntegrationType()).put("sessionId", braintreeFragment.getSessionId()).put(ClientCookie.VERSION_ATTR, "3.0.0").put("platform", "android").toString()));
        } catch (JSONException unused) {
        }
        return jSONObject;
    }
    ...
    public static void setGooglePaymentRequestDefaults(BraintreeFragment braintreeFragment, Configuration configuration, GooglePaymentRequest googlePaymentRequest) {
        ...
        if (googlePaymentRequest.isPayPalEnabled().booleanValue() && configuration.isPayPalEnabled() && !TextUtils.isEmpty(configuration.getPayPal().getClientId())) {
            z = true;
        }
        if (z) {
    ...
    private static boolean validateManifest(Context context) {
        ...
        return activityInfo != null && activityInfo.getThemeResource() == C0964R.C0969style.bt_transparent_activity;
    }
}
