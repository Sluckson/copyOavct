...
public class PdfFormField extends PdfAnnotation {
    ...
    public static final PdfName IF_SCALE_ALWAYS = PdfName.f641A;
    public static final PdfName IF_SCALE_ANAMORPHIC = PdfName.f641A;
    public static final PdfName IF_SCALE_BIGGER = PdfName.f646B;
    public static final PdfName IF_SCALE_NEVER = PdfName.f696N;
    public static final PdfName IF_SCALE_PROPORTIONAL = PdfName.f707P;
    public static final PdfName IF_SCALE_SMALLER = PdfName.f719S;
    ...
    public void setButton(int i) {
        put(PdfName.f678FT, PdfName.BTN);
        if (i != 0) {
            put(PdfName.f675FF, new PdfNumber(i));
        }
    }
    ...
    public static PdfFormField createTextField(PdfWriter pdfWriter, boolean z, boolean z2, int i) {
        PdfFormField pdfFormField = new PdfFormField(pdfWriter);
        pdfFormField.put(PdfName.f678FT, PdfName.f732TX);
        int i2 = 0;
        int i3 = z ? 4096 : 0;
        if (z2) {
            i2 = 8192;
        }
        pdfFormField.put(PdfName.f675FF, new PdfNumber(i3 + i2));
        if (i > 0) {
    ...
    protected static PdfFormField createChoice(PdfWriter pdfWriter, int i, PdfArray pdfArray, int i2) {
        PdfFormField pdfFormField = new PdfFormField(pdfWriter);
        pdfFormField.put(PdfName.f678FT, PdfName.f657CH);
        pdfFormField.put(PdfName.f675FF, new PdfNumber(i));
        pdfFormField.put(PdfName.OPT, pdfArray);
        if (i2 > 0) {
            pdfFormField.put(PdfName.f727TI, new PdfNumber(i2));
        }
        return pdfFormField;
    }
    ...
    public static PdfFormField createSignature(PdfWriter pdfWriter) {
        PdfFormField pdfFormField = new PdfFormField(pdfWriter);
        pdfFormField.put(PdfName.f678FT, PdfName.SIG);
        return pdfFormField;
    }
    ...
    public int setFieldFlags(int i) {
        int i2;
        PdfNumber pdfNumber = (PdfNumber) get(PdfName.f675FF);
        if (pdfNumber == null) {
            i2 = 0;
        } else {
            i2 = pdfNumber.intValue();
        }
        put(PdfName.f675FF, new PdfNumber(i | i2));
        return i2;
    }

    public void setValueAsString(String str) {
        put(PdfName.f736V, new PdfString(str, PdfObject.TEXT_UNICODE));
    }

    public void setValueAsName(String str) {
        put(PdfName.f736V, new PdfName(str));
    }

    public void setValue(PdfSignature pdfSignature) {
        put(PdfName.f736V, pdfSignature);
    }

    public void setDefaultValueAsString(String str) {
        put(PdfName.f669DV, new PdfString(str, PdfObject.TEXT_UNICODE));
    }

    public void setDefaultValueAsName(String str) {
        put(PdfName.f669DV, new PdfName(str));
    }
    ...
    public void setUserName(String str) {
        put(PdfName.f731TU, new PdfString(str, PdfObject.TEXT_UNICODE));
    }

    public void setMappingName(String str) {
        put(PdfName.f729TM, new PdfString(str, PdfObject.TEXT_UNICODE));
    }

    public void setQuadding(int i) {
        put(PdfName.f714Q, new PdfNumber(i));
    }
    ...
    public void setUsed() {
        ...
        if (this.kids != null) {
            ...
            for (int i = 0; i < this.kids.size(); i++) {
                pdfArray.add((PdfObject) ((PdfFormField) this.kids.get(i)).getIndirectReference());
            }
            put(PdfName.KIDS, pdfArray);
        }
        if (this.templates != null) {
            PdfDictionary pdfDictionary = new PdfDictionary();
            for (PdfTemplate resources : this.templates.keySet()) {
                mergeResources(pdfDictionary, (PdfDictionary) resources.getResources());
            }
            put(PdfName.f667DR, pdfDictionary);
        }
    }
    ...
    public static PdfAnnotation shallowDuplicate(PdfAnnotation pdfAnnotation) {
        ...
        if (pdfAnnotation.isForm()) {
            ...
            pdfAnnotation2 = new PdfAnnotation(pdfAnnotation.writer, (Rectangle) null);
        }
