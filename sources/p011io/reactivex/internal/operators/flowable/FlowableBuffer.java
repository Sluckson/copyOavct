...
public final class FlowableBuffer<T, C extends Collection<? super T>> extends AbstractFlowableWithUpstream<T, C> {
    ...
    static final class PublisherBufferExactSubscriber<T, C extends Collection<? super T>> implements FlowableSubscriber<T>, Subscription {
        ...
        public void request(long j) {
            if (SubscriptionHelper.validate(j)) {
                this.f5046s.request(BackpressureHelper.multiplyCap(j, (long) this.size));
            }
        }
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f5046s, subscription)) {
                this.f5046s = subscription;
    ...
    static final class PublisherBufferSkipSubscriber<T, C extends Collection<? super T>> extends AtomicInteger implements FlowableSubscriber<T>, Subscription {
        ...
        public void request(long j) {
            ...
            if (get() != 0 || !compareAndSet(0, 1)) {
                this.f5048s.request(BackpressureHelper.multiplyCap((long) this.skip, j));
                return;
            }
            this.f5048s.request(BackpressureHelper.addCap(BackpressureHelper.multiplyCap(j, (long) this.size), BackpressureHelper.multiplyCap((long) (this.skip - this.size), j - 1)));
        }
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f5048s, subscription)) {
                this.f5048s = subscription;
                this.actual.onSubscribe(this);
            }
        }
    ...
    static final class PublisherBufferOverlappingSubscriber<T, C extends Collection<? super T>> extends AtomicLong implements FlowableSubscriber<T>, Subscription, BooleanSupplier {
            ...
            if (SubscriptionHelper.validate(j)) {
                if (!QueueDrainHelper.postCompleteRequest(j, this.actual, this.buffers, this, this)) {
                    if (this.once.get() || !this.once.compareAndSet(false, true)) {
                        this.f5047s.request(BackpressureHelper.multiplyCap((long) this.skip, j));
                        return;
                    }
                    this.f5047s.request(BackpressureHelper.addCap((long) this.size, BackpressureHelper.multiplyCap((long) this.skip, j - 1)));
                }
            }
        }
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f5047s, subscription)) {
                this.f5047s = subscription;
                this.actual.onSubscribe(this);
            }
        }
