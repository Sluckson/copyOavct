...
import p011io.reactivex.internal.fuseable.SimplePlainQueue;
...
import p011io.reactivex.processors.UnicastProcessor;
import p011io.reactivex.subscribers.DisposableSubscriber;
...
public final class FlowableWindowBoundarySelector<T, B, V> extends AbstractFlowableWithUpstream<T, Flowable<T>> {
    ...
    static final class WindowBoundaryMainSubscriber<T, B, V> extends QueueDrainSubscriber<T, Object, Flowable<T>> implements Subscription {
        ...
        /* renamed from: ws */
        final List<UnicastProcessor<T>> f5154ws;
        ...
        WindowBoundaryMainSubscriber(Subscriber<? super Flowable<T>> subscriber, Publisher<B> publisher, Function<? super B, ? extends Publisher<V>> function, int i) {
            ...
            this.f5154ws = new ArrayList();
            this.windows.lazySet(1);
        }

        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f5153s, subscription)) {
                this.f5153s = subscription;
        ...
        public void onNext(T t) {
            if (!this.done) {
                if (fastEnter()) {
                    for (UnicastProcessor<T> onNext : this.f5154ws) {
                        onNext.onNext(t);
                    }
                    if (leave(-1) == 0) {
                        return;
        ...
        public void drainLoop() {
            ...
            List<UnicastProcessor<T>> list = this.f5154ws;
            int i = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (z && z2) {
                    ...
                    if (windowOperation.f5155w != null) {
                        if (list.remove(windowOperation.f5155w)) {
                            windowOperation.f5155w.onComplete();
                            if (this.windows.decrementAndGet() == 0) {
                                dispose();
                                return;
                            }
        ...
        public void close(OperatorWindowBoundaryCloseSubscriber<T, V> operatorWindowBoundaryCloseSubscriber) {
            ...
            this.queue.offer(new WindowOperation(operatorWindowBoundaryCloseSubscriber.f5152w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    static final class WindowOperation<T, B> {
        ...
        /* renamed from: w */
        final UnicastProcessor<T> f5155w;

        WindowOperation(UnicastProcessor<T> unicastProcessor, B b) {
            this.f5155w = unicastProcessor;
            this.open = b;
        }
    }
    ...
    static final class OperatorWindowBoundaryCloseSubscriber<T, V> extends DisposableSubscriber<V> {
        ...
        /* renamed from: w */
        final UnicastProcessor<T> f5152w;
