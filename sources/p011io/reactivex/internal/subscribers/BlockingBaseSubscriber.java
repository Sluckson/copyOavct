...
public abstract class BlockingBaseSubscriber<T> extends CountDownLatch implements FlowableSubscriber<T> {
    ...
    public final void onSubscribe(Subscription subscription) {
        if (SubscriptionHelper.validate(this.f5355s, subscription)) {
            this.f5355s = subscription;
            if (!this.cancelled) {
                subscription.request(Long.MAX_VALUE);
                if (this.cancelled) {
                    this.f5355s = SubscriptionHelper.CANCELLED;
                    subscription.cancel();
                }
            }
        }
    ...
    public final T blockingGet() {
        if (getCount() != 0) {
            try {
                ...
                Subscription subscription = this.f5355s;
                this.f5355s = SubscriptionHelper.CANCELLED;
