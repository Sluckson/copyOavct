...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f83mX = new float[min];
            this.f84mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f83mX[i2] = fArr[0];
                this.f84mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f83mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f84mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f83mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f84mY[i3] - 1.0f)) <= 1.0E-5d) {
                    int i4 = 0;
                    float f = 0.0f;
                    while (i < min) {
                        float[] fArr2 = this.f83mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb.append(this.f83mX[0]);
            sb.append(",");
            sb.append(this.f84mY[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f83mX[i6]);
            sb.append(",");
            sb.append(this.f84mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f83mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f83mX[i2]) {
                length = i2;
