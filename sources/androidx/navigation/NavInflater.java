...
import androidx.navigation.common.C0558R;
...
public final class NavInflater {
        ...
        while (true) {
            ...
            if (next == 2 && depth <= depth2) {
                ...
                if (TAG_ARGUMENT.equals(name)) {
                    ...
                    TypedArray obtainAttributes = resources.obtainAttributes(attributeSet, C0552R.styleable.NavInclude);
                    ((NavGraph) createDestination).addDestination(inflate(obtainAttributes.getResourceId(C0552R.styleable.NavInclude_graph, 0)));
    ...
    private void inflateArgumentForDestination(@NonNull Resources resources, @NonNull NavDestination navDestination, @NonNull AttributeSet attributeSet, int i) throws XmlPullParserException {
        TypedArray obtainAttributes = resources.obtainAttributes(attributeSet, C0558R.styleable.NavArgument);
        String string = obtainAttributes.getString(C0558R.styleable.NavArgument_android_name);
        if (string != null) {
    ...
    private NavArgument inflateArgument(@NonNull TypedArray typedArray, @NonNull Resources resources, int i) throws XmlPullParserException {
        ...
        boolean z = false;
        builder.setIsNullable(typedArray.getBoolean(C0558R.styleable.NavArgument_nullable, false));
        TypedValue typedValue = sTmpValue.get();
        if (typedValue == null) {
            typedValue = new TypedValue();
            sTmpValue.set(typedValue);
        }
        String string = typedArray.getString(C0558R.styleable.NavArgument_argType);
        int i2 = null;
        NavType fromArgType = string != null ? NavType.fromArgType(string, resources.getResourcePackageName(i)) : null;
        if (typedArray.getValue(C0558R.styleable.NavArgument_android_defaultValue, typedValue)) {
            if (fromArgType == NavType.ReferenceType) {
                ...
                i2 = typedArray.getString(C0558R.styleable.NavArgument_android_defaultValue);
            } else {
    ...
    private void inflateDeepLink(@NonNull Resources resources, @NonNull NavDestination navDestination, @NonNull AttributeSet attributeSet) throws XmlPullParserException {
        TypedArray obtainAttributes = resources.obtainAttributes(attributeSet, C0558R.styleable.NavDeepLink);
        String string = obtainAttributes.getString(C0558R.styleable.NavDeepLink_uri);
        String string2 = obtainAttributes.getString(C0558R.styleable.NavDeepLink_action);
        String string3 = obtainAttributes.getString(C0558R.styleable.NavDeepLink_mimeType);
    ...
    private void inflateAction(@NonNull Resources resources, @NonNull NavDestination navDestination, @NonNull AttributeSet attributeSet, XmlResourceParser xmlResourceParser, int i) throws IOException, XmlPullParserException {
        int depth;
        TypedArray obtainAttributes = resources.obtainAttributes(attributeSet, C0558R.styleable.NavAction);
        int resourceId = obtainAttributes.getResourceId(C0558R.styleable.NavAction_android_id, 0);
        NavAction navAction = new NavAction(obtainAttributes.getResourceId(C0558R.styleable.NavAction_destination, 0));
        NavOptions.Builder builder = new NavOptions.Builder();
        builder.setLaunchSingleTop(obtainAttributes.getBoolean(C0558R.styleable.NavAction_launchSingleTop, false));
        builder.setPopUpTo(obtainAttributes.getResourceId(C0558R.styleable.NavAction_popUpTo, -1), obtainAttributes.getBoolean(C0558R.styleable.NavAction_popUpToInclusive, false));
        builder.setEnterAnim(obtainAttributes.getResourceId(C0558R.styleable.NavAction_enterAnim, -1));
        builder.setExitAnim(obtainAttributes.getResourceId(C0558R.styleable.NavAction_exitAnim, -1));
        builder.setPopEnterAnim(obtainAttributes.getResourceId(C0558R.styleable.NavAction_popEnterAnim, -1));
        builder.setPopExitAnim(obtainAttributes.getResourceId(C0558R.styleable.NavAction_popExitAnim, -1));
