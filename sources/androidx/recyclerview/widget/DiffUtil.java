...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake snake, Snake snake2) {
            int i = snake.f73x - snake2.f73x;
            return i == 0 ? snake.f74y - snake2.f74y : i;
        }
    };
    ...
    public static DiffResult calculateDiff(@NonNull Callback callback, boolean z) {
        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (diffPartial != null) {
                ...
                diffPartial.f73x += range.oldListStart;
                diffPartial.f74y += range.newListStart;
                Range range2 = arrayList3.isEmpty() ? new Range() : (Range) arrayList3.remove(arrayList3.size() - 1);
                range2.oldListStart = range.oldListStart;
                range2.newListStart = range.newListStart;
                if (diffPartial.reverse) {
                    range2.oldListEnd = diffPartial.f73x;
                    range2.newListEnd = diffPartial.f74y;
                } else if (diffPartial.removal) {
                    range2.oldListEnd = diffPartial.f73x - 1;
                    range2.newListEnd = diffPartial.f74y;
                } else {
                    range2.oldListEnd = diffPartial.f73x;
                    range2.newListEnd = diffPartial.f74y - 1;
                }
                arrayList2.add(range2);
                if (!diffPartial.reverse) {
                    range.oldListStart = diffPartial.f73x + diffPartial.size;
                    range.newListStart = diffPartial.f74y + diffPartial.size;
                } else if (diffPartial.removal) {
                    range.oldListStart = diffPartial.f73x + diffPartial.size + 1;
                    range.newListStart = diffPartial.f74y + diffPartial.size;
                } else {
                    range.oldListStart = diffPartial.f73x + diffPartial.size;
                    range.newListStart = diffPartial.f74y + diffPartial.size + 1;
                }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (snake == null || snake.f73x != 0 || snake.f74y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                int i3 = snake.f73x + snake.size;
                int i4 = snake.f74y + snake.size;
                ...
                for (int i5 = 0; i5 < snake.size; i5++) {
                    int i6 = snake.f73x + i5;
                    int i7 = snake.f74y + i5;
        ...
        private boolean findMatchingItem(int i, int i2, int i3, boolean z) {
            ...
            while (i3 >= 0) {
                ...
                int i6 = snake.f73x + snake.size;
                int i7 = snake.f74y + snake.size;
                int i8 = 8;
                if (z) {
        ...
        public void dispatchUpdatesTo(@NonNull ListUpdateCallback listUpdateCallback) {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                Snake snake = this.mSnakes.get(size);
                int i3 = snake.size;
                int i4 = snake.f73x + i3;
                int i5 = snake.f74y + i3;
                if (i4 < i) {
                ...
                for (int i6 = i3 - 1; i6 >= 0; i6--) {
                    if ((this.mOldItemStatuses[snake.f73x + i6] & 31) == 2) {
                        batchingListUpdateCallback.onChanged(snake.f73x + i6, 1, this.mCallback.getChangePayload(snake.f73x + i6, snake.f74y + i6));
                    }
                }
                i = snake.f73x;
                i2 = snake.f74y;
