...
public class TimeStampResponseGenerator {
    ...
    private PKIStatusInfo getPKIStatusInfo() {
        ...
        if (this.statusStrings.size() > 0) {
            aSN1EncodableVector.add(new PKIFreeText((ASN1Sequence) new DERSequence(this.statusStrings)));
        }
        int i = this.failInfo;
        if (i != 0) {
            aSN1EncodableVector.add(new FailInfo(i));
        }
        return new PKIStatusInfo((ASN1Sequence) new DERSequence(aSN1EncodableVector));
    }
    ...
    public TimeStampResponse generate(TimeStampRequest timeStampRequest, BigInteger bigInteger, Date date, String str) throws NoSuchAlgorithmException, NoSuchProviderException, TSPException {
        ...
        if (date != null) {
            try {
                ...
                timeStampResp = new TimeStampResp(getPKIStatusInfo(), ContentInfo.getInstance(new ASN1InputStream((InputStream) new ByteArrayInputStream(this.tokenGenerator.generate(timeStampRequest, bigInteger, date, str).toCMSSignedData().getEncoded())).readObject()));
            } catch (IOException e) {
                ...
                timeStampResp = new TimeStampResp(getPKIStatusInfo(), (ContentInfo) null);
            }
            try {
    ...
    public TimeStampResponse generate(TimeStampRequest timeStampRequest, BigInteger bigInteger, Date date) throws TSPException {
        ...
        if (date != null) {
            try {
                ...
                this.status = 0;
                addStatusString("Operation Okay");
                timeStampResp = new TimeStampResp(getPKIStatusInfo(), ContentInfo.getInstance(new ASN1InputStream((InputStream) new ByteArrayInputStream(this.tokenGenerator.generate(timeStampRequest, bigInteger, date).toCMSSignedData().getEncoded())).readObject()));
            } catch (IOException e) {
                throw new TSPException("Timestamp token received cannot be converted to ContentInfo", e);
            } catch (TSPValidationException e2) {
                this.status = 2;
