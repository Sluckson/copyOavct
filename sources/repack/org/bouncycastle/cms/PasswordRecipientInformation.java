...
public class PasswordRecipientInformation extends RecipientInformation {
    ...
    static {
        ...
        KEYSIZES.put(CMSAlgorithm.DES_EDE3_CBC, new Integer(PsExtractor.AUDIO_STREAM));
        KEYSIZES.put(CMSAlgorithm.AES128_CBC, new Integer(128));
        KEYSIZES.put(CMSAlgorithm.AES192_CBC, new Integer(PsExtractor.AUDIO_STREAM));
    ...
    public RecipientOperator getRecipientOperator(Recipient recipient) throws CMSException, IOException {
        ...
        if (passwordRecipient.getPasswordConversionScheme() == 0) {
            ...
            bArr = ((KeyParameter) pKCS5S2ParametersGenerator.generateDerivedParameters(intValue)).getKey();
        } else {
            PKCS5S2ParametersGenerator pKCS5S2ParametersGenerator2 = new PKCS5S2ParametersGenerator();
            pKCS5S2ParametersGenerator2.init(PBEParametersGenerator.PKCS5PasswordToUTF8Bytes(passwordRecipient.getPassword()), instance3.getSalt(), instance3.getIterationCount().intValue());
            bArr = ((KeyParameter) pKCS5S2ParametersGenerator2.generateDerivedParameters(intValue)).getKey();
        }
        return passwordRecipient.getRecipientOperator(AlgorithmIdentifier.getInstance(instance.getParameters()), this.messageAlgorithm, bArr, this.info.getEncryptedKey().getOctets());
    }
}
