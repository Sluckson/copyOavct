...
public class JDKDSASigner extends SignatureSpi implements PKCSObjectIdentifiers, X509ObjectIdentifiers {
    ...
    public void engineInitSign(PrivateKey privateKey) throws InvalidKeyException {
        ...
        if (privateKey instanceof GOST3410Key) {
            parametersWithRandom = GOST3410Util.generatePrivateKeyParameter(privateKey);
        } else {
            parametersWithRandom = DSAUtil.generatePrivateKeyParameter(privateKey);
        }
    ...
    private byte[] derEncode(BigInteger bigInteger, BigInteger bigInteger2) throws IOException {
        return new DERSequence((ASN1Encodable[]) new DERInteger[]{new DERInteger(bigInteger), new DERInteger(bigInteger2)}).getEncoded(ASN1Encodable.DER);
    }
